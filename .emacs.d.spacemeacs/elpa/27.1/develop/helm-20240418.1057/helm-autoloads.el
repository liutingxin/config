...
disable it if ARG is zero or negative.  If called from Lisp, also enable the
...
If called interactively, enable Helm-Epa mode if ARG is positive, and disable it
...
If called interactively, enable Helm-Ff-Icon mode if ARG is positive, and
disable it if ARG is zero or negative.  If called from Lisp, also enable the
mode if ARG is omitted or nil, and toggle it if ARG is `toggle'; disable the
mode otherwise.
